...
try:
    ...
    try:
        ...
        logger.info("can't import appindicator")
    else:
...
def set_dir_icon(dir_path, icon_type):
    ...
    if icon_type == 'normal':
        ...
        os.system('gvfs-set-attribute -t string "%s" metadata::custom-icon file://%s' % (
...
class JavaAppWatchDog(threading.Thread):
    ...
    def start_java_app(self, args):
        ...
        """
        const_jvm_settings = '-ea -client -Dfile.encoding=UTF-8 -Xmx512M -XX:MinHeapFreeRatio=20 ' \
        ...
        jvm_settings_A = '-Dswt.autoScale=exact -Djava.util.logging.config.file=%s/java.logging.properties ' \
                         '-Dnutstore.config.dir=%s -Dnutstore.resource.dir=%s -Dnutstore.x64=%s' % (
                             conf_dir, conf_dir, dist_dir, is_x86_64())
        ...
        if USE_PYTHON_TRAY:jvm_settings += " --use-python-tray"
        ...
        try:
            ...
            if cmd == 'open':
                ...
                if len(lines) != 4:
                    ...
                    # This line is encoded by base64
                    dir_path = base64.standard_b64decode(payload)
                    icon_type = lines[2]
